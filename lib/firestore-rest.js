...
export async function updateDocument(collectionId, documentId, data) {
  ...
  const url = `${documentsBase()}/${encodeURIComponent(collectionId)}/${encodeURIComponent(documentId)}${mask ? `?${mask}` : ""}`;
  const response = await authorizedFetch(url, {
    method: "PATCH",
    body: JSON.stringify({ fields: toFields(data) }),
  });
  ...
  if (!response.ok) {
    const error = new Error(payload.error?.message || "Firestore document update failed");
    error.code = payload.error?.status || `HTTP_${response.status}`;
    throw error;
  }
...
export async function commitMutations(operations = []) {
  const writes = operations.map((operation) => {
    const name = `${documentsBase()}/${encodeURIComponent(operation.collectionId)}/${encodeURIComponent(operation.documentId)}`;
    ...
    if (operation.type === "create") {
      return {
        update: { name, fields: toFields(operation.data || {}) },
    ...
    if (operation.type === "update") {
      return {
        update: { name, fields: toFields(operation.data || {}) },
        updateMask: { fieldPaths: Object.keys(operation.data || {}) },
      };
    }
  ...
  const response = await authorizedFetch(`${documentsBase()}:commit`, {
    method: "POST",
    body: JSON.stringify({ writes }),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    const error = new Error(payload.error?.message || "Firestore commit failed");
    error.code = payload.error?.status || `HTTP_${response.status}`;
    throw error;
  }
  return payload;
