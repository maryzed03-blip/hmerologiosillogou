...
## Δήλωση κατάθεσης μετά τη συμμετοχή

Για πληρωμένες δράσεις, το confirmation email της συμμετοχής περιλαμβάνει πλέον:

- ποσό συμμετοχής,
- προαιρετικές οδηγίες πληρωμής από `PAYMENT_INSTRUCTIONS`,
- μοναδική αιτιολογία κατάθεσης,
- κουμπί **«Έκανα την κατάθεση»**.

Το κουμπί δεν εγκρίνει την πληρωμή. Δημιουργεί εγγραφή στη συλλογή `paymentDeclarations`, την οποία διαβάζει το Central Hub και εμφανίζει στον Ταμία ως **Προς έλεγχο**.

Πρόσθεσε στο Vercel:

```text
PUBLIC_APP_URL=https://TO-DOMAIN-TOY-HMEROLOGIOY
PAYMENT_INSTRUCTIONS=IBAN / δικαιούχος / σύντομες οδηγίες
```

Τα server endpoints χρειάζονται επίσης τα `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY`, όπως ήδη χρησιμοποιούν οι συμμετοχές/εγκρίσεις.
